package hjsonpp.expand.graphics.g3d;
...
public class RingMesh extends PlanetMesh{
    ...
    public RingMesh(String path, Planet planet, int sides, float radiusIn, float radiusOut, int degrees){
        this.planet = planet;
        this.region = Core.atlas.find(path);
        this.texture = region.texture;

        MeshUtils.begin(sides * 12);

        Vec3 axis = new Vec3(0,1,0).rotate(Vec3.X, degrees);
    ...
    private static Shader shader(){
        return HjsonppShaders.planetTextureShader;
    }

    @Override
    public void preRender(PlanetParams params){
        HjsonppShaders.planetTextureShader.planet = this.planet;

        if(planet == null || planet.solarSystem == null){
            return;
        }
        HjsonppShaders.planetTextureShader.lightDir
                .set(planet.solarSystem.position)
                .sub(planet.position)
                .rotate(Vec3.Y, planet.getRotation())
                .nor();

        HjsonppShaders.planetTextureShader.ambientColor
                .set(planet.solarSystem.lightColor);

        HjsonppShaders.planetTextureShader.alpha = 1f;
    }
    ...
    public void render(PlanetParams params, Mat3D projection, Mat3D transform){
        ...
        shader.setUniformf("u_lightdir", HjsonppShaders.planetTextureShader.lightDir);
        shader.setUniformf("u_ambientColor", HjsonppShaders.planetTextureShader.ambientColor);
        shader.setUniformf("u_alpha", HjsonppShaders.planetTextureShader.alpha);
